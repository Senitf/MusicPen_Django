    ...
    */
    ...
    $.ajax({
      ...
      success: function(response){ 
        canvas_data[canvas_dataCnt++] = response.output;
        tmp = 0
        while(tmp < canvas_dataCnt){
            console.log(tmp);
            console.log(parseInt(tmp / 4) * 2);
            console.log((tmp % 4) * 100 - 100);
            canvas = document.getElementsByTagName("canvas")[(tmp / 4) * 2];
            ctx = canvas.getContext('2d');
            var img = new Image();
            img.onload = function() {
